...
export default function CertificateModal({ name, course, date, onClose }) {
  ...
  const download = async () => {
    ...
    try {
      // Capture the exact rendered preview. A dynamic scale keeps the output
      // print-sharp without creating a differently laid-out off-screen clone.
      await document.fonts?.ready
      await Promise.all([...src.querySelectorAll('img')].map(img => img.decode?.().catch(() => {})))
      const previewWidth = src.getBoundingClientRect().width || 920
      const scale = Math.max(3, Math.min(8, 2800 / previewWidth))
      ...
      pdf.save(`Certificate-${certNumber(course, date)}.pdf`)
    } finally {
      setBusy(false)
    }
  }
