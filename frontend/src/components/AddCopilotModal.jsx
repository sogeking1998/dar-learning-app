...
import { X, Headphones, User, AtSign, Briefcase, Building2, Loader2 } from 'lucide-react'
import GenderSelect from './GenderSelect'
import { createCopilotUser, ADMIN_EMAIL_DOMAIN } from '../adminUsersStore'
import './AddAdminModal.css'

const DIVISIONS = ['PBD', 'LTS', 'AJD', 'Admin']

export default function AddCopilotModal({ onClose, onCreated }) {
  ...
  const submit = async e => {
    ...
    setBusy(true)
    const { error, email } = await createCopilotUser(form)
    setBusy(false)
    if (error) {
    ...
    <div className="aa-overlay" onClick={onClose}>
      <div className="aa-modal" onClick={e => e.stopPropagation()}>
        <header className="aa-head">
          <div className="aa-head-ic"><Headphones size={20} /></div>
          <div className="aa-head-text">
            <h3 className="aa-title">Add Co-pilot</h3>
            <p className="aa-sub">Create a customer-service (co-pilot) account manually</p>
          </div>
        ...
        <form className="aa-form" onSubmit={submit}>
          <div className="aa-field">
            ...
            <div className="aa-input-wrap">
              ...
              <input className="aa-input" value={form.name} onChange={e => set('name', e.target.value)} placeholder="e.g. Ana Reyes" />
            </div>
          </div>
          ...
          <div className="aa-field">
            ...
            <div className="aa-input-wrap">
              ...
              <input className="aa-input" value={form.username} onChange={e => set('username', e.target.value.trim())} placeholder="e.g. copilot1" autoCapitalize="none" />
            </div>
          ...
          <div className="aa-row">
            ...
            <div className="aa-field">
              ...
              <div className="aa-input-wrap">
                ...
                <input className="aa-input" value={form.position} onChange={e => set('position', e.target.value)} placeholder="e.g. Support Specialist" />
              </div>
            </div>
          </div>
          ...
          <div className="aa-actions">
            ...
            <button type="submit" className="aa-btn aa-btn-create" disabled={busy}>
              {busy ? <><Loader2 size={15} className="aa-spin" /> Creating…</> : <><Headphones size={15} /> Create Co-pilot</>}
            </button>
          </div>
        </form>
      </div>
